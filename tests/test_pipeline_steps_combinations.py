...
async def test_pipeline_steps_extract_img_preview_picture():
    ...
    outputs = await step1.process(
        ...
        args={
            ...
            'directory_or_file_name': "test_zip_with_picture/App Center_001.png"
        }
    )

    final_outputs = await step2.process(
        outputs.results,
        args={
            'max_width': 3000,
            'max_height': 3000,
    ...
    output = await anext(final_outputs.results)
    ...
    with pytest.raises(StopAsyncIteration):
        await anext(final_outputs.results)