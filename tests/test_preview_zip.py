...
"""
class TestPreviewZip(unittest.IsolatedAsyncioTestCase):
    async def test_preview_zip_success_from_url(self):
        step = PreviewZip()
    ...
    async def test_preview_zip_success_from_inputs_bytes(self):
    ...
    async def test_preview_zip_success_from_inputs_url(self):
    ...
    async def test_preview_zip_fail_no_inputs_no_args(self):
        step = PreviewZip()

        with self.assertRaisesRegex(ValueError, "No input or arguments were provided to PreviewZip step."):
            async for _ in step.process(None, args={}):
                pass


    async def test_preview_zip_invalid_url(self):
        step = PreviewZip()
    ...
    async with aiohttp.ClientSession() as session:
        async def get_data():
            yield UrlPipelineData(
                "https://github.com/oarepo/oarepo-file-pipeline-server/raw/refs/heads/first-version/tests/test_zip.zip",
                session
            )
