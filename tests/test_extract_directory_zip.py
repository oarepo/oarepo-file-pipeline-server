...
async def test_extract_file_zip_success_from_input_bytes():
    ...
    outputs = await step.process(inputs, args={'directory_or_file_name': 'test_directory_zip/directory2/'})
    output = await anext(outputs.results)
    buffer = io.BytesIO()
    async for content in output:
        buffer.write(content)
...
async def test_extract_file_zip_success_from_input_bytes_1000_files():
    ...
    outputs = await step.process(inputs, args={'directory_or_file_name': '1000_files/1000_files_zip'})
...
async def test_extract_directory_zip_success_from_url_1000_files():
    ...
    outputs = await step.process(None,
                           args={
                               ...
                               "directory_or_file_name": "1000_files/1000_files_zip"
                           })
